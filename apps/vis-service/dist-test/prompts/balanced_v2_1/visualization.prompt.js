import { BALANCED_V2_1_PHASE_1, BALANCED_V2_1_PHASE_2_HEADER, RENOVATION_CONTINUITY, FURNISHING_STRATEGY, OBJECT_PLACEMENT_LOGIC, STYLE_PRIORITY, REALISM_CONSTRAINT, DESIGN_RULES_V2_1, RENOVATION_FEASIBILITY_RULES, LIGHTING_PRESERVATION_AND_NORMALIZATION, BALANCED_V2_1_PROTOCOL_RIGID_BASE, BALANCED_V2_1_PROTOCOL_RIGID_APERTURE_LOCK, BALANCED_V2_1_PROTOCOL_SURFACE_ONLY_TRANSFORM, INFLUENCE_PRIORITIZE_PRESET_STYLE, INFLUENCE_PRIORITIZE_MOOD_BOARD, INFLUENCE_BALANCE_STYLE_AND_MOOD_BOARD, INFLUENCE_PRESET_STYLE_ONLY, INSTRUCTION_INTEGRATE_FURNITURE, DEFAULT_USER_REQUEST, } from './visualization.constants.js';
// ── helpers ───────────────────────────────────────────────────────────────────
...
const getProtocol = (protocol) => {
    switch (protocol) {
        case 'rigid_aperture_lock': return BALANCED_V2_1_PROTOCOL_RIGID_APERTURE_LOCK;
        case 'surface_only_transform': return BALANCED_V2_1_PROTOCOL_SURFACE_ONLY_TRANSFORM;
        default: return BALANCED_V2_1_PROTOCOL_RIGID_BASE;
    }
};
// ── builder ───────────────────────────────────────────────────────────────────
export const buildVisualizationPrompt = (params) => {
    ...
    const structuralPart = BALANCED_V2_1_PHASE_1;
    // Phase 2 assembly order (differs from balanced_v2 by insertion of OBJECT_PLACEMENT_LOGIC):
    //   Header → Renovation Continuity → Furnishing Strategy → Object Placement Logic [NEW]
    //   → Style Definition → Style Priority → Realism Constraint → Design Rules
    //   → Renovation Feasibility → Lighting Preservation & Normalization → Structural Protocol
    const stylePart = [
        BALANCED_V2_1_PHASE_2_HEADER
        ...
        STYLE_PRIORITY,
        REALISM_CONSTRAINT,
        DESIGN_RULES_V2_1.replace(/\{\{ROOM_TYPE\}\}/g, roomType),
