// ============================================================
// BALANCED V4.0 — INJECTION LAYER
// Implements the template-driven prompt engine for pipelineMode 'balanced_v4_0'.
//
// Derived from balanced_v3_0/visualization.prompt.ts. All v3.0 injection
// logic is preserved. V4.0 additions:
//   - hasInjectedItem param: controls INJECTED_ITEM_AUDIT_BLOCK injection
//   - buildConstraintHierarchyBlock exported (assembled in service, not here)
//   - buildInfluencePrompt: pass-through from v3.0
//
// Processing order (unchanged from v3.0 §4.1):
...
//   8. Inject INJECTED_ITEM_AUDIT_BLOCK (v4.0 addition)
...
// ============================================================
import { TEMPLATE_VERSION, BALANCED_V4_0_STRUCTURAL_PART, BALANCED_V4_0_STYLE_TEMPLATE, INJECTED_ITEM_AUDIT_TEXT, INFLUENCE_PRIORITIZE_PRESET_STYLE, INFLUENCE_PRIORITIZE_MOOD_BOARD, INFLUENCE_BALANCE_STYLE_AND_MOOD_BOARD, INFLUENCE_PRESET_STYLE_ONLY, DEFAULT_USER_REQUEST, } from './visualization.constants.js';
...
// ── Aperture sanitizer ────────────────────────────────────────────────────────
// Carried over from v3.0. aperture_look has no injection point in v4.0 —
...
export const sanitizeApertureLook = (raw) => {
    ...
    console.warn(`[balanced_v4_0] aperture_look is set ("${raw.slice(0, 60)}...") but has no injection point in template v${TEMPLATE_VERSION}. Value is excluded from the rendered prompt.`);
...
export const buildVisualizationPrompt = (params) => {
    ...
    let rendered = BALANCED_V4_0_STYLE_TEMPLATE
    ...
    // ── Step 8: Inject INJECTED_ITEM_AUDIT_BLOCK (v4.0) ──────────────────────
    // Present when an item is injected; empty string + clean whitespace when not.
    if (hasInjectedItem) {
    ...
    const structuralPart = BALANCED_V4_0_STRUCTURAL_PART;
    // ── Step 10: Return ───────────────────────────────────────────────────────
    return {
        structuralPart,
        stylePart: rendered,
