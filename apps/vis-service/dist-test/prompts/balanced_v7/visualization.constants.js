...
export { BALANCED_V5_STRUCTURAL_PART, BALANCED_V5_STYLE_TEMPLATE, INJECTED_ITEM_BLOCK_HEADER, INJECTED_ITEM_AUDIT_TEXT, buildMoodboardScopeBlock, buildRenovationAnchorsBlock, V5_MOODBOARD_INFLUENCE_STATEMENT, INFLUENCE_PRESET_STYLE_ONLY, DEFAULT_USER_REQUEST, } from '../shared/canonicalPromptPrimitives.js';
...
export const buildConstraintHierarchyBlock = (injectedItemCount, hasRenovationAnchors = false, hasHardAGTFacts = false) => {
    ...
    const agtTierLine = '  Verified hard facts from the structural assessment above are TIER 1 constraints - they supersede model visual judgment.';
    const lines = base.split('\n');
    const insertionIndex = lines.findIndex((line) => line.includes('All elements defined in Phase 1: Architectural Anchoring are immutable.'));
    if (insertionIndex === -1) {
        return base;
    }
