...
import { buildConstraintHierarchyBlock, INJECTED_ITEM_BLOCK_HEADER, } from '../../../prompts/balanced_v4_0/visualization.constants.js';
import { buildVisualizationPrompt, buildInfluencePrompt, } from '../../../prompts/balanced_v4_0/visualization.prompt.js';
...
export const generateVisualization = async (params) => {
    const { roomImage, roomType, stylePreset, moodBoardImages, textPrompt, styleInfluence, isRefinement, previousResultImage, } = params;
    ...
    if (injectedItems.length > 1) {
        throw new Error(`balanced_v4_0 supports a maximum of 1 injected item. ` +
    ...
    if (firstPart?.inlineData?.data) {
        return {
            ...
            debug: {
                ...
                requestStructure: parts.map(p => 'text' in p
