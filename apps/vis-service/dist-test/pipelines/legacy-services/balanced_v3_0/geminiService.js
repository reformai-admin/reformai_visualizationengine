import { loadEnvFile } from 'node:process';
import { GoogleGenAI, Modality } from "@google/genai";
import { buildVisualizationPrompt, buildInfluencePrompt, buildFurniturePrompt } from '../../../prompts/balanced_v3_0/visualization.prompt.js';
if (!process.env.K_SERVICE) {
    try {
        loadEnvFile();
    }
    catch {
        // .env file not found — rely on environment variables already set
    }
}
const API_KEY = process.env.API_KEY;
if (!API_KEY) {
    throw new Error('API_KEY environment variable is not set.');
}
const ai = new GoogleGenAI({ apiKey: API_KEY });
// Config flag: balanced_v3_0 is Real Mode — style objects cannot mutate architecture
const ALLOW_ARCHITECTURAL_STYLE_MUTATION = false;
const bufferToGenerativePart = (file) => ({
    inlineData: { data: file.buffer.toString('base64'), mimeType: file.mimetype },
});
export const generateVisualization = async (params) => {
    const { roomImage, roomType, stylePreset, moodBoardImages, furnitureImage, textPrompt, styleInfluence, isRefinement, previousResultImage, } = params;
    const model = 'gemini-2.5-flash-image';
    const influencePrompt = buildInfluencePrompt(moodBoardImages.length, styleInfluence, stylePreset.name);
    const furniturePrompt = buildFurniturePrompt(!!furnitureImage, roomType);
    const { structuralPart, stylePart, rawApertureLook, safeApertureLook, apertureSanitized, stagingDensityTier, } = buildVisualizationPrompt({
        roomType,
        stylePreset,
        textPrompt,
    });
    // Request structure: original → structural anchor → style → moodboard → furniture → original (re-anchor)
    const parts = [
        bufferToGenerativePart(roomImage),
        { text: structuralPart },
        { text: stylePart },
        ...moodBoardImages.map(bufferToGenerativePart),
    ];
    if (furnitureImage) {
        parts.push(bufferToGenerativePart(furnitureImage));
    }
    if (furniturePrompt) {
        parts.push({ text: furniturePrompt });
    }
    if (influencePrompt) {
        parts.push({ text: influencePrompt });
    }
    if (isRefinement && previousResultImage) {
        parts.push(bufferToGenerativePart(previousResultImage));
    }
    // Final structural anchor — structural sandwich preserved
    parts.push(bufferToGenerativePart(roomImage));
    const response = await ai.models.generateContent({
        model,
        contents: { parts },
        config: { responseModalities: [Modality.IMAGE] },
    });
    const firstPart = response.candidates?.[0]?.content?.parts?.[0];
    if (firstPart?.inlineData?.data) {
        return {
            image: firstPart.inlineData.data,
            debug: {
                pipelineMode: 'balanced_v3_0',
                templateVersion: '3.1',
                structuralPart,
                stylePart,
                styleObject: stylePreset,
                structuralProtocol: stylePreset.pipeline_config?.structural_protocol ?? 'rigid_base',
                stagingDensityTier,
                requestStructure: parts.map(p => 'text' in p ? 'TEXT' : 'IMAGE'),
                // Aperture sanitizer metadata (not injected in v3.0)
                rawApertureLook,
                safeApertureLook,
                apertureSanitized,
                allowArchitecturalStyleMutation: ALLOW_ARCHITECTURAL_STYLE_MUTATION,
            },
        };
    }
    const finishReason = response.candidates?.[0]?.finishReason;
    throw new Error(`No image returned by Gemini.${finishReason ? ` Finish reason: ${finishReason}` : ' Response may have been blocked.'}`);
};
//# sourceMappingURL=geminiService.js.map