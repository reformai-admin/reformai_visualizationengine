...
test('V7 hierarchy includes AGT hard-fact line only when hard facts exist', () => {
    ...
    assert.equal(
        withoutHardFacts.includes('Verified hard facts from the structural assessment above'),
        false,
    );
    assert.equal(
        withHardFacts.includes('Verified hard facts from the structural assessment above'),
        true,
    );
});
