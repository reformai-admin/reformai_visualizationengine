...
import {
    TEMPLATE_VERSION,
    PIPELINE_MODE,
    BALANCED_V4_0_STRUCTURAL_PART,
    BALANCED_V4_0_STYLE_TEMPLATE,
...
export const sanitizeApertureLook = (
    ...
    console.warn(
        `[balanced_v4_0] aperture_look is set ("${raw.slice(0, 60)}...") but has no injection point in template v${TEMPLATE_VERSION}. Value is excluded from the rendered prompt.`,
    );
...
export interface BalancedV4_0PromptParams {
    ...
    textPrompt: string;
    hasInjectedItem: boolean; // v4.0: controls INJECTED_ITEM_AUDIT_BLOCK injection
}
...
export const buildVisualizationPrompt = (params: BalancedV4_0PromptParams): BuiltBalancedV4_0Prompt => {
