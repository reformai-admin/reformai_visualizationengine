...
import { resolveHandlerMode, resolvePipelineMode } from './pipeline-routing.js';

test('defaults to balanced_v7 when pipeline mode is omitted', () => {
    assert.equal(resolvePipelineMode(undefined), 'balanced_v7');
});

test('balanced_v6 resolves to its own dedicated handler', () => {
    assert.equal(resolveHandlerMode('balanced_v6'), 'balanced_v6');
});
