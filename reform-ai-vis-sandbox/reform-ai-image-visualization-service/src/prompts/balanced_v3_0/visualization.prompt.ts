...
import {
    TEMPLATE_VERSION,
    PIPELINE_MODE,
    BALANCED_V3_0_STRUCTURAL_PART,
    BALANCED_V3_0_STYLE_TEMPLATE,
...
export const sanitizeApertureLook = (
    ...
    console.warn(
        `[balanced_v3_0] aperture_look is set ("${raw.slice(0, 60)}...") but has no injection point in template v${TEMPLATE_VERSION}. Value is excluded from the rendered prompt.`,
    );
...
export interface BuiltBalancedV3_0Prompt {
    ...
    stylePart: string;
    // Aperture sanitizer metadata — not injected in v3.0, exposed for debug
    rawApertureLook: string;
    safeApertureLook: string;
    apertureSanitized: boolean;
    // Staging density metadata
    stagingDensityTier: string;
}

// ── Builder ───────────────────────────────────────────────────────────────────

export const buildVisualizationPrompt = (params: BalancedV3_0PromptParams): BuiltBalancedV3_0Prompt => {
