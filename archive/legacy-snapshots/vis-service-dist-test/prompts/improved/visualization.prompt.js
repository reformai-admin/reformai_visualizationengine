import { MAIN_PROMPT_ROOM_REDESIGN, MAIN_PROMPT_IMAGE_REFINEMENT, INFLUENCE_PRIORITIZE_PRESET_STYLE, INFLUENCE_PRIORITIZE_MOOD_BOARD, INFLUENCE_BALANCE_STYLE_AND_MOOD_BOARD, INFLUENCE_PRESET_STYLE_ONLY, INSTRUCTION_INTEGRATE_FURNITURE, DEFAULT_USER_REQUEST, PHASE_ANCHOR_STRUCTURE, PHASE_ANCHOR_STRUCTURE_V2, PHASE_ANCHOR_STYLE, BEHAVIORAL_CONSTRAINTS, PROTOCOL_RIGID_BASE, PROTOCOL_RIGID_APERTURE_LOCK, PROTOCOL_SURFACE_ONLY_TRANSFORM, GLOBAL_STYLE_CONSTRAINTS, LIGHTING_GEOMETRY_SAFETY, } from './visualization.constants.js';
const buildBulletedStyleBlock = (modelInputs) => {
    const lines = [
        '**STYLE DEFINITION:**',
        'Style Materials:',
...
const getProtocolInstructions = (protocol) => {
    switch (protocol) {
...
const buildImprovedPrompt = (params) => {
    const { roomType, stylePreset, textPrompt, phaseAnchoringV2, } = params;
...
const buildBaselinePrompt = (params) => {
    const { isRefinement, roomType, stylePreset, influencePrompt, textPrompt, furniturePrompt, } = params;
