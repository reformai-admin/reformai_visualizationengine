...
export const buildAGTConstraintBlock = (agt) => {
    const hardLines = [];
    const advisoryLines = [];
    // window_count
    if (agt.window_count.enforcement === 'hard') {
        ...
        const anchors = agt.window_count.spatialAnchors?.length
            ...
            : '';
        hardLines.push(`EXACTLY ${count} window${count !== 1 ? 's' : ''} exist in this room.${anchors}`);
        if (count === 0) {
            hardLines.push('No window openings are present. Do not introduce any new window, skylight, glazed opening, or faux exterior aperture.');
        }
    }
    else if (agt.window_count.enforcement === 'advisory') {
        advisoryLines.push(`Window count: approximately ${agt.window_count.displayValue} observed - verify against source image.`);
    }
    // door_count
    if (agt.door_count.enforcement === 'hard') {
        ...
        const anchors = agt.door_count.spatialAnchors?.length
            ...
            : '';
        hardLines.push(`EXACTLY ${count} door${count !== 1 ? 's' : ''} exist in this room.${anchors}`);
    }
    else if (agt.door_count.enforcement === 'advisory') {
        advisoryLines.push(`Door count: approximately ${agt.door_count.displayValue} observed - verify against source image.`);
    }
    // has_ceiling_fixture
    if (agt.has_ceiling_fixture.enforcement === 'hard') {
        const present = agt.has_ceiling_fixture.displayValue === 'PRESENT';
        hardLines.push(present
        ...
        hardLines.push('Do not introduce new permanent ceiling structure such as beams, coffers, plank systems, dropped soffits, or skylight framing.');
    }
    else if (agt.has_ceiling_fixture.enforcement === 'advisory') {
        advisoryLines.push(`Ceiling fixture: ${agt.has_ceiling_fixture.displayValue} (advisory - model judgment governs if ambiguous in source).`);
    }
    // has_built_in_niches
    if (agt.has_built_in_niches.enforcement === 'hard') {
        ...
        advisoryLines.push(`Built-in niches: ${agt.has_built_in_niches.displayValue} (advisory).`);
    }
    const lines = [
    ...
    if (hardLines.length > 0) {
        ...
        lines.push('');
        lines.push('VISIBILITY CONTRACT: All confirmed hard-fact architectural elements above must remain ' +
        ...
        lines.push('Structural extraction confidence was insufficient to produce hard facts for this image. ' +
    ...
    if (agt.camera_perspective.enforcement === 'advisory') {
        lines.push('');
        lines.push(`Camera perspective: ${agt.camera_perspective.displayValue} (advisory - composition governs).`);
    }
    return lines.join('\n').trimEnd();
};
//# sourceMappingURL=agt-constraint.js.map